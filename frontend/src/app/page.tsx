...
const INITIAL_MOCK_STORES: Store[] = [
  // WORLD CUP ATHLETICS (Zone A)
  {
    ...
    location: { type: 'Point', coordinates: [121.501, 31.24] },
...
export default function Home() {
    ...
    async function syncRmnData() {
      try {
        ...
        const resA = await fetch(
          `${API_BASE}/api/rmn/products?zoneId=Zone_A&query=${encodeURIComponent(queryA)}`
        );
        const dataA = resA.ok ? await resA.json() : null;

        const resB = await fetch(
          `${API_BASE}/api/rmn/products?zoneId=Zone_B&query=${encodeURIComponent(queryB)}`
        );
  ...
  const handleSimulateSurge = async (zone: 'A' | 'B') => {
    ...
    setActiveZone(zone);
    const coords: [number, number] = zone === 'A' ? [121.501, 31.24] : [121.515, 31.245];
    ...
    if (backendOnline) {
      try {
        ...
        addLocalLog(
          `[ERROR] Direct surge POST failed: ${err.message}. Reverting to local simulation.`
        );
      }
    } else {
      // Execute offline simulation behavior
      addLocalLog(
        `[SURGE_REQ] Manual surge triggered for Zone ${zone} at [${coords[0]}, ${coords[1]}]`
      );
      addLocalLog(
        `[GEOSPATIAL] Geospatial Query: searching within 500m of [${coords[0]}, ${coords[1]}]`
      );
      ...
      addLocalLog(
        `[GEOSPATIAL] Geospatial Query: found ${matchingStores.length} stores inside boundaries`
      );
      ...
      matchingStores.forEach((store) => {
        if (store.current_stock > store.target_stock) {
          addLocalLog(
            `[INVENTORY] Store '${store.name}' surplus detected: stock=${store.current_stock}, target=${store.target_stock}`
          );
          ...
          addLocalLog(
            `[ACTION] Stock decremented for '${store.name}': ${oldStock} -> ${store.current_stock} (-${dec})`
          );
          ...
          addLocalLog(
            `[INVENTORY] Store '${store.name}' has no surplus. Stock levels within safe targets.`
          );
        }
      });
  ...
  const getCalculatedDiscount = (storeName: string, itemName: string, zone: 'A' | 'B') => {
    ...
    if (sentimentResult) {
      ...
      if (itemName.toLowerCase().includes('jersey') || itemName.toLowerCase().includes('cap')) {
        if (
          ...
          q.includes('shirt') ||
          msg.includes('jersey') ||
          msg.includes('apparel') ||
          msg.includes('athletics')
        ) {
          sentimentBoost = 15;
        }
      }
      if (
        itemName.toLowerCase().includes('ball') ||
        itemName.toLowerCase().includes('toy') ||
        itemName.toLowerCase().includes('game')
      ) {
        if (
          q.includes('kid') ||
          q.includes('child') ||
      ...
      if (
        itemName.toLowerCase().includes('bottle') ||
        itemName.toLowerCase().includes('water') ||
        itemName.toLowerCase().includes('snack') ||
        itemName.toLowerCase().includes('fan') ||
    ...
    const currentStoreItem = currentStoresList.find(
      (s) => s.name === storeName && s.item === itemName
    );
    if (currentStoreItem && query.trim().length >= 2) {
      ...
      if (isSearchMatch) {
        const siblings = currentStoresList.filter(
          (s) =>
  ...
  const handleSearchSentiment = async (zone: 'A' | 'B') => {
    ...
    const coords = zone === 'A' ? [121.501, 31.24] : [121.515, 31.245];
    ...
    if (backendOnline) {
      ...
      setTimeout(() => {
        const mockResult = {
          ...
          empatheticMessage:
            query.toLowerCase().includes('hot') || query.toLowerCase().includes('tired')
  ...
  const fetchRmnZoneData = async (zone: 'Zone_A' | 'Zone_B', query = '') => {
    try {
      const response = await fetch(
        `${API_BASE}/api/rmn/products?zoneId=${zone}&query=${encodeURIComponent(query)}`
      );
      if (response.ok) {
        const data = await response.json();
        if (zone === 'Zone_A') {
  ...
  const handleRmnSearch = async (zone: 'Zone_A' | 'Zone_B') => {
    ...
    if (lowerQuery.includes('poncho')) {
      ...
      lowerQuery.includes('mascot') ||
      lowerQuery.includes('toy') ||
      lowerQuery.includes('plush')
    ) {
      mpn = 'FIFA-MASCOT';
    } else if (
      lowerQuery.includes('brew') ||
    ...
    if (backendOnline) {
      ...
      const updated = mockStoresRef.current.map((store) => {
        if (store.name.toLowerCase() === storeName.toLowerCase()) {
          ...
          mockPromotionsRef.current = mockPromotionsRef.current
            .map((promo) => {
  ...
  const renderLogLine = (log: LogEntry, index: number) => {
    ...
    const timeStr = new Date(parseTimestamp(log.timestamp)).toLocaleTimeString([], {
      hour12: false,
    });
    ...
    return (
      <div
        key={index}
        className="py-1.5 border-b border-zinc-900/60 hover:bg-zinc-900/20 transition-colors flex items-start gap-2.5 text-[11px] font-mono leading-relaxed"
      >
        <span className="text-zinc-500 shrink-0 select-none">{timeStr}</span>
        {badge && (
          <span
            className={`px-1.5 py-0.5 rounded text-[9px] font-bold shrink-0 tracking-wider uppercase ${badgeColor}`}
          >
            {badge}
          </span>
        )}
    ...
    <div className="flex flex-col h-screen w-screen bg-zinc-950 text-zinc-100 overflow-hidden font-sans antialiased selection:bg-zinc-800 selection:text-white">
      ...
      <header className="shrink-0 flex items-center justify-between border-b border-zinc-900 bg-zinc-900/40 backdrop-blur-md px-6 py-4">
        ...
        <div className="flex items-center space-x-3 font-mono text-[11px]">
          ...
          >
            <span
              className={`h-1.5 w-1.5 rounded-full ${backendOnline ? 'bg-emerald-400' : 'bg-amber-400 animate-pulse'}`}
            />
              ...
              <div className="relative border border-zinc-900 rounded bg-zinc-950/40 p-2 flex items-center justify-center aspect-[4/3] max-h-[300px]">
                ...
                <svg className="w-full h-full text-zinc-700" viewBox="0 0 400 300">
                  ...
                  <text
                    ...
                    y="245"
                    fontSize="9"
                    fill={activeZone === 'A' ? '#34d399' : '#52525b'}
                    textAnchor="middle"
                    fontWeight="bold"
                  >
                    ZONE A // WEST WING
                  ...
                  <g onClick={() => handleSimulateSurge('A')} className="cursor-pointer">
                    <rect
                      ...
                      stroke="#3f3f46"
                      strokeWidth="1"
                      className="hover:stroke-zinc-400 transition-colors"
                    />
                    <text
                      x="62.5"
                      y="82"
                  ...
                  <text
                    ...
                    y="245"
                    fontSize="9"
                    fill={activeZone === 'B' ? '#34d399' : '#52525b'}
                    textAnchor="middle"
                    fontWeight="bold"
                  >
                    ZONE B // EAST WING
                ...
                <div className="grid grid-cols-2 gap-2 text-zinc-300">
                  <div>
                    ...
                    <span className="text-zinc-100 font-bold">
                      {activeZone ? `Zone ${activeZone}` : 'Standby'}
                    </span>
                  </div>
                  <div>
                    Range:{' '}
                    <span className="text-zinc-100 font-bold">{activeZone ? '500m' : '0m'}</span>
                  </div>
                  <div className="col-span-2">
                    Coordinates:{' '}
                    <span className="text-zinc-100 font-bold">
                      {lastScanCoords
                        ? `${lastScanCoords[0].toFixed(3)}, ${lastScanCoords[1].toFixed(3)}`
                        : 'Standby'}
                    </span>
                  </div>
                </div>
            ...
            <div className="flex-1 p-4 overflow-y-auto space-y-4">
              {promotions.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-center p-6 border border-dashed border-zinc-900 rounded bg-zinc-950/10">
                  <svg
                    className="h-8 w-8 text-zinc-700 mb-2"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
                ...
                [...promotions].reverse().map((promo) => {
                  ...
                  const initialStock = store
                    ? store.name.includes('World Cup')
                      ? 150
                      : store.name.includes('Champions')
                        ? 120
                        : store.name.includes('Fan')
                          ? 80
                          : 60
                    : 150;
                    ...
                    >
                      ...
                      <div className="space-y-1">
                        <div className="flex items-center justify-between text-[9px] text-zinc-500 font-mono">
                          ...
                          <span
                            className={isCritical ? 'text-rose-400 font-bold' : 'text-zinc-355'}
                          >
                            {isCritical ? 'CRITICAL MINIMUM' : 'ADEQUATE'} ({currentStock} /{' '}
                            {initialStock})
                          </span>
                        </div>
                      ...
                      <div className="mt-3 relative border border-dashed border-zinc-800 bg-zinc-900/20 rounded py-2 flex flex-col items-center justify-center">
                        <div className="text-[8px] text-zinc-500 font-bold uppercase tracking-wider font-mono">
                          REDEMPTION CODE
                        </div>
          ...
          <div className="shrink-0 flex items-center justify-between border-b border-zinc-900 pb-3">
            <div>
              ...
              <p className="text-[10px] text-zinc-500 font-mono mt-0.5">
                Simulate consumer experience across Zone A and Zone B. Search logs trigger yield
                adjustments, and purchases run ACID inventory transactions.
              </p>
            </div>
            {backendOnline && (
            ...
            <div className="flex flex-col border border-zinc-900 bg-zinc-900/10 rounded-lg p-5 space-y-4">
              <div className="flex items-center justify-between border-b border-zinc-900 pb-2 flex-wrap gap-2">
                ...
                <div className="flex items-center space-x-2 text-[10px] bg-zinc-950/60 border border-zinc-900 rounded px-2 py-1">
                  ...
                  <button
                    onClick={() =>
                      handleRmnSimulateTraffic('Zone_A', Math.max(0, rmnVisitorsA - 1))
                    }
                  ...
                  <span className="w-4 text-center font-bold text-zinc-200 font-mono">
                    {rmnVisitorsA}
                  </span>
                  <button
                ...
                >
                  ...
                  <div className="flex items-center justify-between mb-2">
                    <span className="bg-gradient-to-r from-violet-500 to-indigo-500 text-white text-[8px] font-extrabold uppercase px-1.5 py-0.5 rounded tracking-wider shadow-sm font-mono">
                      {rmnSponsoredA.tenantId === 'coop_partnership'
                        ? '★ CO-OP PARTNERSHIP // AD WINNER'
                        : '★ Sponsored // Ad Auction Winner'}
                    </span>
                    <span className="text-[8px] text-zinc-500 font-mono">
                      CPC: ${rmnSponsoredA.actual_cpc.toFixed(2)} | Ad Rank:{' '}
                      {rmnSponsoredA.ad_rank.toFixed(1)}
                    </span>
                  </div>
                  <div className="space-y-1">
                    ...
                    <p className="text-[10px] text-zinc-400 leading-normal">{rmnSponsoredA.body}</p>
                  </div>
                    ...
                    <div className="mt-3 bg-zinc-900/40 border border-zinc-800/40 rounded p-2.5 flex items-center justify-between">
                      <div>
                        ...
                        <div className="mt-1 flex items-baseline space-x-1.5">
                          ...
                          {rmnSponsoredA.product.googleMerchantFields.sale_price <
                            rmnSponsoredA.product.googleMerchantFields.base_price && (
                      ...
                      >
                        {rmnSponsoredA.product.inventory_metrics.availableStock <= 0
                          ? 'OUT OF STOCK'
                          : 'BUY OFFER'}
                      </button>
                    </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {rmnProductsA
                  .filter(
                    (p) =>
                      !rmnSponsoredA ||
                      p.googleMerchantFields.g_mpn !==
                        rmnSponsoredA.product?.googleMerchantFields.g_mpn
                  )
                  .map((product) => {
                      ...
                      >
                        ...
                        <div className="space-y-1">
                          ...
                          <h4 className="text-xs font-bold text-zinc-200">
                            {product.googleMerchantFields.title}
                          </h4>
                          <p className="text-[9px] text-zinc-500 line-clamp-1">
                            {product.googleMerchantFields.description}
                          </p>
                        </div>
                        ...
                        <div className="mt-4 space-y-1">
                          <div className="flex justify-between text-[9px] font-mono text-zinc-500">
                            ...
                            <span
                              className={
                                isCritical ? 'text-rose-400 font-semibold' : 'text-zinc-350'
                              }
                            >
                              {avail} (Buffer: {buffer})
                            </span>
                ...
                <div className="p-3 text-[10px] text-zinc-300 space-y-2 max-h-[190px] overflow-y-auto leading-relaxed">
                  {rmnLedgerA.length === 0 ? (
                    <div className="text-zinc-650 italic">
                      No auctions executed yet. Run a search to trigger.
                    </div>
                  ) : (
                    <div className="space-y-2">
                      ...
                      {rmnLedgerA.map((cand) => {
                        ...
                        return (
                          <div
                            key={cand.campaignId}
                            className="flex flex-col border-b border-zinc-900/30 pb-1.5 last:border-0 last:pb-0"
                          >
                            <div className="flex items-center justify-between">
                              <span
                                className={`${isWinner ? 'text-violet-300 font-extrabold' : 'text-zinc-455 font-bold'}`}
                              >
                                {cand.campaignId}
                              </span>
                              <span
                                className={`px-1 rounded text-[8px] font-bold tracking-wider ${
                                  isWinner
                                    ? 'bg-emerald-950/60 text-emerald-400 border border-emerald-800/30'
                                    : isRunnerUp
                                      ? 'bg-amber-950/60 text-amber-400 border border-amber-800/30'
                                      : isDisqualified
                                        ? 'bg-rose-950/30 text-rose-500/70 border border-rose-900/10'
                            ...
                            {!isDisqualified ? (
                              <div className="text-zinc-400 mt-0.5 text-[9px]">
                                ...
                                | Ad Rank:{' '}
                                <span className="text-zinc-200 font-bold">
                                  {cand.adRank.toFixed(2)}
                                </span>
                              </div>
                            ) : (
                              <div className="text-rose-500/60 italic mt-0.5 text-[9px]">
                                Disqualified:{' '}
                                {cand.status === 'DISQUALIFIED_STOCK_SHORTAGE'
                                  ? 'Out of Stock (Safety Buffer)'
                                  : 'No Rank'}
                              </div>
                            )}
                          </div>
                        ...
                        if (winner) {
                          ...
                          return (
                            <div className="mt-2 pt-2 border-t border-zinc-800/60 text-[8px] text-zinc-500 space-y-1 bg-zinc-950/30 p-2 rounded">
                              <div className="text-zinc-400 font-bold uppercase tracking-wider text-[8px]">
                                Programmatic Settlement (Vickrey):
                              </div>
                              {runnerUp ? (
                                <div>
                                  CPC = (Runner Up Ad Rank / (Winner QS * Multiplier)) + $0.01{' '}
                                  <br />
                                  CPC = ({runnerUp.adRank.toFixed(2)} / ({winner.qs.toFixed(1)} *{' '}
                                  {rmnMultiplierA})) + $0.01 ={' '}
                                ...
                                <div>
                                  CPC = Max Bid or Default Flat Minimum ={' '}
                                  <span className="text-emerald-400 font-extrabold">
                                    ${billedCpc.toFixed(2)}
                                  </span>
                                </div>
            ...
            <div className="flex flex-col border border-zinc-900 bg-zinc-900/10 rounded-lg p-5 space-y-4">
              <div className="flex items-center justify-between border-b border-zinc-900 pb-2 flex-wrap gap-2">
                ...
                <div className="flex items-center space-x-2 text-[10px] bg-zinc-950/60 border border-zinc-900 rounded px-2 py-1">
                  ...
                  <button
                    onClick={() =>
                      handleRmnSimulateTraffic('Zone_B', Math.max(0, rmnVisitorsB - 1))
                    }
                    className="w-4 h-4 flex items-center justify-center border border-zinc-800 bg-zinc-900 hover:bg-zinc-800 text-zinc-300 rounded cursor-pointer select-none"
                  >
                    -
              ...
              {rmnSponsoredB && (
                ...
                >
                  ...
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-[8px] text-zinc-500 font-mono">
                      CPC: ${rmnSponsoredB.actual_cpc.toFixed(2)} | Ad Rank:{' '}
                      {rmnSponsoredB.ad_rank.toFixed(1)}
                    </span>
                  </div>
                  <div className="space-y-1">
                    <h4 className="text-xs font-bold text-violet-300 leading-snug">
                    ...
                    <p className="text-[10px] text-zinc-400 leading-normal">{rmnSponsoredB.body}</p>
                  </div>
                    ...
                    <div className="mt-3 bg-zinc-900/40 border border-zinc-800/40 rounded p-2.5 flex items-center justify-between">
                      <div>
                        ...
                        <div className="mt-1 flex items-baseline space-x-1.5">
                          ...
                          {rmnSponsoredB.product.googleMerchantFields.sale_price <
                            rmnSponsoredB.product.googleMerchantFields.base_price && (
                            <span className="text-[9px] text-zinc-650 line-through">
                      ...
                      <button
                        ...
                        className="shrink-0 py-1 px-3 border border-violet-650 hover:border-violet-500 bg-violet-600/10 hover:bg-violet-600/25 text-violet-300 text-[10px] font-bold rounded cursor-pointer transition-all disabled:opacity-40 disabled:cursor-not-allowed active:scale-[0.98]"
                      >
                        {rmnSponsoredB.product.inventory_metrics.availableStock <= 0
                          ? 'OUT OF STOCK'
                          : 'BUY OFFER'}
                      </button>
                    </div>
                ...
                {rmnProductsB
                  .filter(
                    (p) =>
                      !rmnSponsoredB ||
                      p.googleMerchantFields.g_mpn !==
                        rmnSponsoredB.product?.googleMerchantFields.g_mpn
                  )
                  .map((product) => {
                    const originalPrice = product.googleMerchantFields.base_price;
                    const salePrice = product.googleMerchantFields.sale_price;
            ...
            <main className="flex-1 flex flex-col p-5 overflow-y-auto space-y-6">
              <div className="shrink-0 flex items-center justify-between border-b border-zinc-900 pb-3">
                <div>
                  ...
                  <p className="text-[10px] text-zinc-500 font-mono mt-0.5">
                    Set max CPC bids, budget limits, target geofence zones, and audience keywords.
                    Update creative copy and view real-time delivery performance.
                  </p>
                </div>
                <div className="flex bg-zinc-950/60 p-1 rounded-md border border-zinc-900">
                  ...
                  {merchantCampaigns.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 gap-5">
                      {merchantCampaigns.map((camp) => {
                        ...
                        const ctr =
                          camp.impressions > 0 ? (camp.clicks / camp.impressions) * 100 : 0;
                            ...
                            <div className="flex items-start justify-between border-b border-zinc-900 pb-3 mb-4">
                              <div>
                                <div className="flex items-center gap-2">
                                  ...
                                  <span
                                    className={`px-1.5 py-0.5 rounded text-[8px] font-bold tracking-wider uppercase font-mono ${
                            ...
                            {!isEditing ? (
                              <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-[11px] leading-relaxed">
                                  <div>
                                    <span className="text-zinc-500 uppercase tracking-wider font-bold block mb-1">
                                      Targeting Criteria
                                    </span>
                                    <div className="space-y-1 font-mono">
                                      <div>
                                        ...
                                        <span className="text-zinc-300">
                                          {(camp.targetingCriteria.targetZones || []).join(', ')}
                                        </span>
                                      </div>
                                      <div>
                                        Keywords:{' '}
                                        <span className="text-zinc-300">
                                          {(
                                            camp.targetingCriteria.audienceContextVectors || []
                                          ).join(', ')}
                                        </span>
                                      </div>
                                    </div>
                                ...
                                <div className="border border-zinc-900 bg-zinc-950/30 rounded p-3 relative">
                                  <span className="absolute -top-2 left-3 bg-zinc-950 px-1 text-[8px] text-zinc-500 font-bold tracking-wider uppercase font-mono">
                                    Ad Creative Preview
                                  </span>
                                  <div className="space-y-1 mt-1">
                                    <h4 className="text-xs font-bold text-violet-300">
                                      {camp.creativeAsset.headline}
                                    </h4>
                                    <p className="text-[10px] text-zinc-400">
                                      {camp.creativeAsset.body}
                                    </p>
                                  </div>
                                </div>
                              </div>
                              ...
                              <div className="space-y-4 text-xs">
                                <div className="grid grid-cols-2 gap-4">
                                  <div>
                                    <label className="block text-zinc-500 font-bold uppercase tracking-wider text-[9px] mb-1">
                                      Max Bid Per Click ($)
                                    </label>
                                    <input
                                      type="number"
                                  ...
                                  <div>
                                    ...
                                    <input
                                      ...
                                      onChange={(e) =>
                                        setEditDailyBudget(parseFloat(e.target.value))
                                      }
                                      className="w-full px-2.5 py-1.5 rounded bg-zinc-950 border border-zinc-850 focus:outline-none focus:border-zinc-700 text-zinc-200 font-mono"
                                    />
                                  </div>
                                ...
                                <div>
                                  <label className="block text-zinc-500 font-bold uppercase tracking-wider text-[9px] mb-1">
                                    Targeting Keywords / Context Vectors (Comma Separated)
                                  </label>
                                  <input
                                    type="text"
                                    value={editKeywords}
                ...
                <div className="space-y-6">
                  <section className="border border-zinc-900 bg-zinc-900/10 rounded-lg p-5">
                    ...
                    <form onSubmit={handleCreateJointCampaign} className="space-y-4 text-xs">
                      ...
                      <div>
                        <label className="block text-zinc-500 font-bold uppercase tracking-wider text-[9px] mb-2">
                          Select Partner Tenants (Min 2)
                        </label>
                        <div className="space-y-1.5 font-mono">
                          {[
                            ...
                            return (
                              <label
                                key={t.id}
                                className="flex items-center space-x-2 text-zinc-300 cursor-pointer select-none"
                              >
                                <input
                                  type="checkbox"
                                  checked={isChecked}
                  ...
                  <section className="border border-zinc-900 bg-zinc-900/10 rounded-lg p-5 text-[11px] leading-relaxed text-zinc-400 space-y-2">
                    <span className="font-bold text-zinc-200 uppercase block tracking-wider text-[9px] mb-2 font-mono">
                      ⚡ How Joint Billing Works
                    </span>
                    <p>
                      Programmatic RMN Vickrey Settlement splits the billed CPC cost equally between
                      all participating partner campaigns.
                    </p>
                    <p>
                      For example, if the winning co-op ad CPC is settled at $1.50, then $0.75 is
                      atomically deducted from both partner merchant's primary campaign budgets.
                    </p>
                    <p>
                      This stimulates cross-tenant co-opetitive marketing, bringing down
                      single-merchant customer acquisition costs while preserving campaign margins.
                    </p>
                  </section>
                </div>
              </div>
